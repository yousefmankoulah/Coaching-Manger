...
export const getCustomerInfo = async (req, res, next) => {
    try {
        if (req.user.id === req.params.customerId || req.user.isAdmin === true || req.user.id === req.params.userId) {
            const customerInfo = await Customer.findOne({ customerId: req.params.customerId });
...
export const updateCutomerInfo = async (req, res, next) => {
    ...
    if (req.user.id === req.params.customerId) {
        try {
            const updatedCustomerInfo = await Customer.findByIdAndUpdate(req.params._id,
                {
                $set: {
                customerCurrentWeight,
                customerTargetWeight,
...
export const addCustomerExerciesInfo = async (req, res, next) => {
    const { date, time, maxCarringWeight, minCarringWeight, timeSpend } = req.body;
    if (req.user.id === req.params.customerId) {
...
export const getAllCustomerExerices = async (req, res, next) => {
    try {
        if (req.user.id === req.params.customerId || req.user.isAdmin === true || req.user.id === req.params.userId) {
            const customerExercies = await CustomerExercies.find({ customerId: req.params.customerId });
...
export const getCustomerExerciesBySetExerciesId = async (req, res, next) => {
    try {
        if (req.user.id === req.params.customerId || req.user.isAdmin === true || req.user.id === req.params.userId) {
            const customerExercies = await CustomerExercies.find({ setExerciesToCustomerId: req.params.setExerciesToCustomerId });
            res.status(200).json(customerExercies);
        } else {
            next(errorHandler(401, 'You are not allowed to perform this action'));
        }
...
export const updateCustomerExerciesInfo = async (req, res, next) => {
    ...
    if (req.user.id === req.params.customerId) {
        try {
            const updatedCustomerExerciesInfo = await CustomerExercies.findByIdAndUpdate(req.params._id, {
                $set: {
                    date,
                    time,
                    maxCarringWeight,
...
export const deleteCustomerExercies = async (req, res, next) => {
    if (req.user.id === req.params.customerId || req.user.isAdmin === true) {
        try {
            const deletedCustomerExercies = await CustomerExercies.findByIdAndDelete(req.params._id);
